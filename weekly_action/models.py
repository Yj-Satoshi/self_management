...
class WeeklyAction(models.Model):
    ...
    goal_action = models.CharField(
        verbose_name='目標達成の為のアクション', help_text='1週単位で実践する行動', max_length=255)
    why_select_action = models.TextField(
        verbose_name='アクション設定理由', help_text='なぜそのアクションで目標が達成できるのか？アクション設定("P")の確認（未設定可）',
        max_length=500, null=True, blank=True)
    score = models.IntegerField(
        verbose_name='自己評価', null=True,
        help_text='アクションを終えたら入力。中間達成度チェック("C")（範囲１〜５, 数字が高い程、評価良）',
        validators=[clean_score], blank=True)
    after_memo = models.TextField(
        verbose_name='後書きメモ',
        max_length=500, null=True,
        help_text='評価の際の反省点などの後から振り返るメモ。中間達成度チェック("C")・次のアクション("A")の確認（未設定可）',
        blank=True)
